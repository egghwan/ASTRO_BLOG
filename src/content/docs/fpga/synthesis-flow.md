...
그런데 이 과정에서 의문이 들어야 한다. 내가 제어하지도 않았는데 카운터는 이미 증가하고 있고, 또 현재 카운터가 어떤지 파형을 통해서만 볼 수 있다. 따라서 우리는 **CPU에서 이 `counter`를 제어하고, 또 현재 값을 읽을 수 있다면** 더 고도화된 제품을 만들 수 있을 것이다.
이 단순한 `counter`를 이제 **CPU에서 켜고/끄고 값을 읽는** 형태로 발전시키고 싶다면, 앞 글에서 다룬 **AXI4-Lite 슬레이브 레지스터맵**이 바로 그 다음 단계다. 지금은 PL 안에서 혼자 도는 카운터지만, 레지스터맵을 앞에 붙이면 CPU가 `CNT_START`에 1을 써서 켜고, `CNT_VALUE`를 읽어 지금 값을 가져올 수 있다. 오늘 만든 `count`가, 레지스터맵 뒤에 붙는 PL 하드웨어(`hw_cnt_value`)로 그대로 확장되는 것이다.