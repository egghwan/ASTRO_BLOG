...
스펙만 필요한 사람을 위해 먼저 싣는다. 각 항목의 근거는 본문에 있고, 모든 숫자는 [재현 스크립트](#부록-a-재현-스크립트)로 직접 확인할 수 있다.

**Catmull-Rom Fractional-Delay Interpolator — 4-tap cubic, Farrow 구조**

| 항목 | 값 | 조건 / 비고 | 상태 |
|---|---|---|---|
| **정격 대역** | **B/Fs ≤ 0.35** | 상한 0.5(나이퀴스트)의 70% | ✅ 측정 |
| **Interpolation SNR** | **31.0 ± 0.3 dB** | B/Fs=0.3125, RRC α=0.25 신호 | ✅ 측정 |
| — linear 대비 | **+10.0 ± 0.1 dB** | 동일 조건 | ✅ 측정 |
| **Passband droop** | **−0.35 dB** (f ≤ 0.1875)<br>−2.52 dB (f ≤ 0.3125) | μ 전 구간 최악값 | ✅ 측정 |
| **지연 오차** | **7.71 % Ts** | 대역 내 최악값. linear와 **동일** | ✅ 측정 |
| **Image 억압** | 1차 **17.8** / 2차 **44.3** / 3차 **57.4 dB** | 복제본 대역 내 최악값 | ✅ 측정 |
| **주파수별 SNR** | 68 / 35 / 15 / 7 dB<br>@ f/Fs = 0.05 / 0.156 / 0.3125 / 0.417 | 신호 가정 없는 응용 무관 곡선 | ✅ 측정 |
| 곱셈기 | 3 (Horner ×μ) | branch는 shift-add로 곱셈기 0 | ⚠️ 추정 |
| 메모리 | 0 | 계수 테이블 불필요 | ✅ 구조상 |
| Latency / Throughput | 8 cycle / 1 sample-per-cycle | 8단 파이프라인 | ⚠️ 추정 |
| 목표 Fmax | 245.76 MHz | RFSoC ZCU208 기준 | ❌ 미검증 |

:::note[검증 상태 표기]
이 문서는 주장마다 근거 수준을 표시한다.
- ✅ **측정** — 재현 스크립트로 확인됨. 독자가 직접 돌려볼 수 있다.
- ⚠️ **추정** — 구조에서 논리적으로 따라나오지만 합성/실측으로 확인하지 않았다.
- ❌ **미검증** — 목표치일 뿐 확인되지 않았다.

기술문서에서 이 셋을 섞으면 안 된다. 특히 아래 항목은 **아직 확인하지 않았음**을 명시해 둔다.

| 미검증 항목 | 왜 중요한가 |
|---|---|
| 합성 결과 (Fmax/LUT/FF/DSP) | Vivado를 돌리지 않았다. 하드웨어 수치는 전부 추정 |
| Lagrange 4-tap 대비 성능 | "왜 Lagrange가 아닌가"의 근거가 아직 정성적 추측이다 |
| 폐루프(STR) implementation loss | 단품 SNR이 시스템 성능으로 얼마나 번역되는지 미확인 |
:::
...
|---|---|---|---|---|
| 1 | **Passband droop** | 신호 크기가 얼마나 깎이나 | **−0.35 dB** (평탄부)<br>−2.5 dB (대역 끝) | −1.6 dB<br>−5.1 dB |
| 2 | **지연 오차** | μ만큼 정확히 밀리나 | 7.7% Ts | 7.7% Ts |
| 3 | **Image 억압** | 복제본을 얼마나 지우나 | 1차 **17.8 dB**<br>2차 **44.3 dB**<br>3차 57.4 dB | 16.6 dB<br>32.2 dB<br>40.3 dB |
| 4 | **Interpolation SNR** | **종합 성적** (실신호 기준) | **31.0 dB** | 21.0 dB |

**읽는 법**: 1–3은 진단 지표(어디가 아픈지), 4가 종합 성적이다. 실제 설계 판단은 4번 하나로 하고, 1–3은 그 원인을 설명한다. 결론부터 말하면 **linear 대비 +10 dB(오차 전력 1/10)** 이고, 이것이 "왜 4-tap cubic을 쓰는가"의 답이다.

:::tip[지표를 읽을 때 반드시 함께 봐야 할 것]
**① 조건 없는 숫자는 의미가 없다.** 위 표의 값은 전부 "B/Fs = 0.3125인 신호" 기준이다. 신호가 대역을 덜 쓰면(예: 4 samples/symbol) 같은 커널이 훨씬 좋은 점수를 받는다 — 5장 끝의 곡선에서 임의의 대역에 대한 값을 읽을 수 있다.
...
| 항목 | 값 | 상태 |
|---|---|---|
| 쓸 수 있는 한계 | **B/Fs ≤ 0.35** (상한 0.5의 70%) — 그 이상은 이 커널로 불가능(물리적 한계) | ✅ 측정 |
| 하드웨어 (2편) | 곱셈기 **3개**, 메모리 0, 8단 파이프라인, 매 클럭 1샘플 | ⚠️ 추정 |
| 목표 동작 주파수 | 245.76 MHz (RFSoC ZCU208) | ❌ 합성 미실시 |
...
⚠️ **다만 이 근거는 아직 정성적이다.** 같은 4-tap인 Lagrange와 **성능을 나란히 재보지 않았다.** 그래서 지금 상태로는 두 가지를 구분할 수 없다 — CR의 31 dB가 4-tap의 한계에 가까운 것인지, 아니면 Lagrange가 몇 dB 더 나오는데 계수 편의를 위해 그만큼 손해 본 것인지. 후자라면 "성능 N dB를 주고 곱셈기 12개를 샀다"는 **트레이드오프**로 서술해야 정확하다. 이 측정은 다음 편의 숙제로 남긴다.
...
![진폭 응답: μ별 |H(f)| 곡선](../../../assets/catmull-rom/fig1_magnitude.png)

μ를 몇 개 골라 **|H(f)|** — 앞서 구한 H의 크기, 즉 "주파수 f의 성분이 몇 배로 나오나" — 를 그린 것이다. 이상적이면 크기가 1배(=0 dB)여야 하므로 모든 곡선이 0 dB 수평선이어야 하는데, 주파수가 올라갈수록 처진다(droop). 파란 음영은 **신호가 실제로 차지하는 대역**인데, 여기서 이 IP의 가장 중요한 파라미터가 등장한다.
...
측정 결과(μ 전 구간에서 최악값): 신호가 대부분의 에너지를 갖는 구간(≤0.1875·Fs)에서 **−0.35 dB(진폭 4% 감소)**, 대역 맨 끝(0.3125·Fs)까지 보면 **−2.53 dB(25% 감소)**. 대역 가장자리에서는 무시 못 할 처짐이 있다는 것도 정직하게 기록해 둔다.

같은 조건에서 **linear는 −1.60 dB / −5.09 dB**다. 평탄부에서 약 4.5배(1.25 dB), 대역 끝에서 2.5 dB 차이 — **여기가 cubic이 실제로 이기는 지점**이다. 4-2에서 보겠지만 지연 특성은 둘이 동일하므로, 종합 성적 +10 dB는 사실상 전부 이 진폭 차이에서 나온다.

### 4-2. 지연이 정확한가 — 지연 오차

![위상 지연: 목표 μ 대비 오차](../../../assets/catmull-rom/fig2_delay.png)

위 그래프는 각 μ에 대해 "실제로 몇 샘플만큼 밀렸나"를 주파수별로 그린 것이다. 점선(지시한 μ)에 딱 붙어 있어야 이상적인데, 고주파로 갈수록 벗어난다. **이 부품의 본업이 '정확한 지연'이므로 이 지표가 가장 본질적**이다.

측정: 대역 내 최악 **7.7% Ts**. 여기서 **Ts는 샘플 주기**(샘플과 샘플 사이의 시간)다. 즉 "μ=0.3만큼 밀어라"라고 지시했는데 실제로는 0.377만큼 밀리는 정도의 오차가 최악의 경우 발생한다는 뜻이다.
...
**함의가 크다.** cubic이 linear보다 좋은 이유가 "타이밍이 더 정확해서"가 아니라 **전적으로 진폭(4-1의 droop)에서 온다**는 뜻이다. 종합 성적 +10 dB의 출처도 진폭이다. 이 구분을 알면 잘못된 처방을 피할 수 있다 — 대칭 커널을 쓰는 한 tap을 늘려도 이 지표는 개선되지 않는다. 지연 오차를 줄이는 유일한 길은 **대역을 덜 쓰는 것**(오버샘플링을 늘리는 것)이다.
:::
...
![연속 커널 FT와 image 억압](../../../assets/catmull-rom/fig7_image_rejection.png)

그림은 커널이 주파수별로 신호를 얼마나 통과시키는지 보여준다. 초록 음영(원본 대역)에서는 높게(통과), 주황 음영(복제본이 있는 자리)에서는 낮게(차단) 지나가야 좋은 커널이다. 검은 점선은 **이상적인 목표**로, 대역 안은 0 dB로 그대로 통과시키고 나이퀴스트(0.5·Fs)부터는 완전히 0으로 떨어지는 벽(brickwall) 모양이다 — 복제본을 남김없이 지우는 이 특성은 무한히 긴 필터로만 가능해서, 실제 커널은 여기에 도달할 수 없다.
...
"image 억압 44 dB"만 쓰면 리뷰에서 "1차는요?"라는 질문에 무너진다. 세 차수를 나란히 적고, 종합 우위는 5장의 interpolation SNR(+10 dB)로 보이는 것이 정직하고 오히려 강하다. 그리고 몇 차 복제본까지 신경 써야 하는지는 **뒷단 회로가 정한다** — 보간기 출력을 바로 저역 필터에 넣는다면 1차만 중요하고, 광대역으로 쓴다면 고차까지 본다.
:::
...
- **③ μ를 1/64 단위로 제한하는 이유**: μ를 아무 실수나 뽑으면 그 지점의 참값을 구하려고 또 보간을 해야 한다 — 보간기를 채점하려고 보간을 쓰는 순환 논리다. 1/64 단위로 제한하면 정답이 그리드 위에 정확히 놓여 있어 **그냥 꺼내 읽으면** 된다.
...
:::note[① 신호 생성부의 실제 코드]
대표 예제의 신호는 모뎀 파형(RRC 펄스 성형된 QPSK)이다. 다른 응용이라면 이 부분만 바꾸면 되고, 조건은 하나 — **대역폭 B 안에 정확히 갇힌 신호**여야 한다.

```matlab
syms = sign(randn(1,Nsym));                    % ±1 심볼
imp  = zeros(1,Nh); imp(1:sps*R:end) = syms;   % 심볼 위치에 임펄스
% 주파수 영역에서 RRC² (송신 필터 × 수신 정합필터) 곱해 대역 제한
fax = [0:Nh/2-1, -Nh/2:-1]*(R/Nh);  af = abs(fax);  Rs = 1/sps;
G = zeros(1,Nh);  G(af <= (1-alpha)*Rs/2) = 1;
tr = af > (1-alpha)*Rs/2 & af <= (1+alpha)*Rs/2;
G(tr) = sqrt(0.5*(1+cos(pi/(alpha*Rs)*(af(tr)-(1-alpha)*Rs/2))));
s_hi = real(ifft(fft(imp).*G.*G));
s_hi = 0.9*s_hi/max(abs(s_hi));                % 클리핑 방지 여유
```

`G`가 RRC 필터의 주파수 응답이고, 이걸 두 번 곱하는 건 송신기와 수신기 양쪽 필터를 모두 통과한 상태를 만들기 위해서다. 요점은 **FFT로 대역을 칼같이 잘랐다**는 것 하나다.
:::
...
| 항목 | 수식 | 값 |
|---|---|---|
| 신호 전력 | $\sum \text{truth}^2$ | 819.70 |
| 오차 전력 | $\sum e^2$ | 0.6456 |
| 비율 | $819.70 / 0.6456$ | **1,270 배** |
| SNR | $10\log_{10}(1{,}270)$ | **31.0 dB** |

**즉 31 dB는 "신호 전력이 오차 전력의 약 1,270배"라는 뜻이다.** 전력이 아니라 진폭(크기)으로 환산하면 √1270 ≈ 36배, 뒤집으면 **오차가 신호의 2.8% 수준**이다. 같은 실험에서 linear는 21 dB — 오차 전력이 10배 크고, 진폭으로는 9.0%다.

| 지표 | Catmull-Rom | Linear | 차이 |
|---|---|---|---|
| Interpolation SNR (모뎀 신호, B/Fs=0.3125) | **31.0 ± 0.3 dB** | 21.0 ± 0.2 dB | **+10.0 ± 0.1 dB** |
| 같은 말 — 오차가 신호의 몇 %인가 | **2.8%** | 9.0% | 약 3.2배 정확 |

:::note[± 는 왜 붙나 — 정직한 유효숫자]
이 실험은 **랜덤 심볼 2000개**로 만든 신호를 쓰므로, 시드를 바꾸면 값이 조금씩 달라진다. 시드 10개로 돌려본 결과다.

| | 평균 | 표준편차 | 범위 |
|---|---|---|---|
| CR SNR | 30.98 dB | 0.27 | 30.35 – 31.39 |
| linear SNR | 21.00 dB | 0.19 | 20.61 – 21.32 |
| **차이** | **9.99 dB** | **0.10** | 9.73 – 10.08 |

**절대값은 ±0.3 dB 흔들리지만 차이는 ±0.1 dB로 훨씬 안정적**이다. 같은 신호로 두 커널을 채점하니 신호의 변동이 양쪽에서 상쇄되기 때문이다.

그래서 이 문서는 "31.1 dB"처럼 소수점 첫째 자리까지 단정하지 않고 **31.0 ± 0.3 dB**로 쓴다. 유효숫자를 과장하면, 다른 시드로 돌려본 독자가 30.4 dB를 보고 "재현 실패"로 오해한다. 설계 판단에 쓸 값은 **흔들림이 적은 "차이(+10 dB)"** 쪽이다.
:::

:::note[4장에서는 15 dB였는데 왜 31 dB인가?]
같은 커널인데 앞뒤로 다른 점수가 나와 혼란스러울 수 있다. **시험 문제가 다르기 때문**이다.

| 숫자 | 무엇을 잰 건가 |
|---|---|
| **15 dB** (4장, Fig3) | 대역 **끝 주파수(0.3125·Fs)의 톤 하나** — 커널이 가장 못하는 지점 |
| **23.5 dB** | 대역 0–0.3125를 **고르게 채운** 신호 (평탄 스펙트럼) |
| **31.0 dB** (이 절) | **실제 모뎀 신호** — 대역 끝에는 에너지가 거의 없다 |

셋 다 맞는 값이고 용도가 다르다. 15 dB는 **최악점**(보수적 하한), 31.0 dB는 **실제로 겪는 성능**이다. 실신호가 더 좋은 점수를 받는 이유는 단순하다 — RRC 성형된 신호는 스펙트럼이 대역 끝으로 갈수록 잦아들어서, **커널이 못하는 구간을 거의 쓰지 않는다**. 반대로 대역 끝까지 꽉 채워 쓰는 신호라면 23.5 dB 쪽에 가까워진다.

설계 문서에는 **어느 조건의 값인지 반드시 함께 적어야** 한다. "SNR 31 dB"만 쓰면 나중에 다른 신호로 재본 사람이 "왜 안 나오지?"가 된다.
:::

이 **+10 dB가 "왜 linear가 아니라 cubic인가"에 대한 최종 답**이다. 4-1, 4-2, 4-3의 개별 지표(droop, delay, image)는 진단용이고, 그것들이 신호 스펙트럼으로 가중되어 합쳐진 종합 성적이 이 한 줄이다.

![주파수별 SNR — 응용 무관 데이터시트 곡선](../../../assets/catmull-rom/fig3_tone_snr.png)

마지막으로 이 그림이 **이 IP의 진짜 데이터시트**다. 특정 신호 가정 없이 "주파수 f의 단일 톤을 넣으면 SNR이 얼마인가"를 전 주파수에 대해 그렸다.
...
$$
\text{SNR}(f) = -10\log_{10}\Big(\big\langle |E(f,\mu)|^2 \big\rangle_\mu\Big)
$$

첫 식의 $E$는 "실제 응답 − 이상적 응답", 즉 오차다. 둘째 식의 $\langle\cdot\rangle_\mu$는 μ 전 구간 평균을 뜻하고, 입력 톤의 크기가 1이라 SNR 분자(신호 전력)가 1이 되어 생략됐다.

시뮬레이션 없이 커널 수식만으로 나오는 값이라, **신호를 가정하지 않는다**는 게 핵심이다. 어떤 응용이든 사용자는 자기 신호의 대역 끝을 x축에서 찾아 y값을 읽으면 된다.

| 응용 | 대역 끝 f/Fs | 이 곡선에서 읽은 SNR |
|---|---|---|
| 10배 오버샘플 계측 | 0.05 | 68 dB — 사실상 투명 |
| 모뎀 4 sps | 0.156 | 약 36 dB |
| 모뎀 2 sps (대표 예제) | 0.3125 | 15 dB (대역 끝 worst) → 스펙트럼 가중 종합 31 dB |
| 오디오 20k/48k | 0.417 | 약 7 dB — **이 커널로는 부족**, 더 긴 필터나 2단 구성 필요 |

마지막 줄이 중요하다. 데이터시트는 잘 되는 영역만이 아니라 **안 되는 영역**도 알려줘야 한다. 이 곡선에서 정격 대역(**B ≤ 0.35·Fs**)이 나온다.
...
| 왜 Catmull-Rom인가 | 중심차분 기울기 선택 → 계수가 2의 거듭제곱 조합 → **곱셈기가 사라짐**(2편). 성능은 linear +10 dB. 절대 최적이 아니라 자원 대비 성능의 한 지점 |
| 왜 Farrow 구조인가 | μ가 매번 바뀌어도 곱셈 3번(Horner)이면 계산 끝 |
| 성능을 뭘로 재나 | 진단 3종: 진폭 처짐 / 지연 오차 / 복제본 억압 ─ 종합 1개: interpolation SNR (정답 아는 실험) |
| 이 설계의 성적 (B=0.3125) | droop −0.35 dB(평탄부) / 지연 오차 7.7% Ts / image 억압 17.8·44.3·57.4 dB / **SNR 31.0 dB (linear +10 dB)** / 정격 B ≤ 0.35·Fs |
| cubic의 이득은 어디서 오나 | **전적으로 진폭**. 지연 특성은 linear와 동일하다(둘 다 대칭 커널 = 선형 위상) |
| 아직 확인 못 한 것 | ⚠️ Lagrange 4-tap 대비 성능 / ❌ 합성 실측치(Fmax·LUT·DSP) / ⚠️ 폐루프 implementation loss |
...
| `B` | 0.3125 | = (1+α)/(2·sps), 신호 대역 B/Fs |
| `B_FLAT` | 0.1875 | = (1−α)/(2·sps), RRC 평탄부 끝 |
...
```
   ...
   PASS  oracle SNR CR          계산=   31.0369  기대=  31.100
   PASS  oracle SNR lin         계산=   20.9562  기대=  21.100
...
| `make_test_signal()` | 5장 note. RRC-QPSK 신호 생성 |
...
def make_test_signal(rng):
    """RRC 성형된 QPSK(I-rail). 주파수영역에서 대역 B로 정확히 잘라 만든다.
       -> 대역제한 신호이므로 고밀도 그리드의 각 점이 '참값'이 된다."""
    n_lo = NSYM * SPS
    Nh   = n_lo * R
    ...
    s_hi = np.real(np.fft.ifft(np.fft.fft(imp) * G * G))   # RRC² = Tx × matched Rx
    return 0.9 * s_hi / np.max(np.abs(s_hi))               # 클리핑 방지 헤드룸

def oracle_experiment(rng):
    s_hi = make_test_signal(rng)
    x_lo = s_hi[::R]                       # 시험지: 64개당 1개
    n = np.arange(2, len(x_lo)-2)          # 채점 지점 (양끝 tap 부족분 제외)

    # μ 는 1/64 의 배수 중에서 무작위로 → 참값이 그리드 위에 정확히 존재
    mu_q = rng.integers(0, R, size=n.size)
    mu_f = mu_q / R
    truth = s_hi[n*R + mu_q]               # 감춰둔 정답지 조회

    y_cr = cr_eval(x_lo[n-1], x_lo[n], x_lo[n+1], x_lo[n+2], mu_f)
    y_ln = (1-mu_f)*x_lo[n] + mu_f*x_lo[n+1]
...
def main():
    ...
    checks += [("oracle SNR CR", o['cr']['snr'], 31.1, 0.5),
               ("oracle SNR lin", o['lin']['snr'], 21.1, 0.5)]
