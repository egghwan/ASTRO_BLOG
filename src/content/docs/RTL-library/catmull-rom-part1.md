...
| **Interpolation SNR** | **23.7 ± 0.2 dB** | B/Fs=0.3125, 평탄 스펙트럼(응용 무관 기준) | ✅ 측정 |
| — linear 대비 | **+6.9 ± 0.1 dB** | 동일 조건 | ✅ 측정 |
| — 모뎀 예시 (RRC α=0.25) | 31.0 dB (linear +10.0) | 대역 끝 에너지가 적어 유리한 특수 신호 | ✅ 측정 |
...
| 4 | **Interpolation SNR** | **종합 성적** (평탄 스펙트럼) | **23.7 dB** | 16.8 dB |
...
⚠️ **다만 이 근거는 아직 정성적이다.** 같은 4-tap인 Lagrange와 **성능을 나란히 재보지 않았다.** 그래서 지금 상태로는 두 가지를 구분할 수 없다 — CR의 23.7 dB가 4-tap의 한계에 가까운 것인지, 아니면 Lagrange가 몇 dB 더 나오는데 계수 편의를 위해 그만큼 손해 본 것인지. 후자라면 "성능 N dB를 주고 곱셈기 12개를 샀다"는 **트레이드오프**로 서술해야 정확하다. 이 측정은 2편에서 다룬다.
...
| 숫자 | 시험 신호 | 성격 |
|---|---|---|
| **15 dB** (4장, Fig3) | 대역 **끝 주파수(0.3125·Fs)의 톤 하나** | 최악점 — 오차가 가장 큰 주파수 |
| **23.7 dB** (이 절, 주 지표) | 대역 0–0.3125를 **고르게 채운 평탄 스펙트럼** | 대역을 최대로 쓰는 보수적 실신호 |
| **31.0 dB** | **RRC 모뎀 신호** | 대역 끝 에너지가 적어 유리한 특수 응용 |

셋 다 맞는 값이다. **이 데이터시트가 23.7 dB를 주 지표로 쓰는 이유**는 범용 IP이기 때문이다 — 사용자가 어떤 스펙트럼을 넣을지 모르므로, 대역을 고르게 쓰는 보수적 신호로 재는 게 안전하다.
...
| 2 sps 대역 (기준 예제) | 0.3125 | 15 dB (대역 끝 worst) → 평탄 스펙트럼 종합 23.7 dB (모뎀 RRC면 31 dB) |
...
| 이 설계의 성적 (B=0.3125) | droop −0.35 dB(평탄부) / 지연 오차 7.7% Ts / image 억압 17.8·44.3·57.4 dB / **SNR 23.7 dB (linear +6.9 dB, 평탄 스펙트럼)** / 정격 B ≤ 0.35·Fs |
