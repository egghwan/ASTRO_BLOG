---
title: AXI4-Lite 슬레이브 레지스터맵 lib_axil_regmap 완전 분석
...
:::note[`volatile`을 왜 붙이나]
위 C 코드에서 `volatile`은 "이 주소의 값은 컴파일러 모르게 하드웨어가 바꿀 수 있으니, **읽기/쓰기를 최적화로 생략하거나 합치지 말라**"는 표시이다. 예를 들어 카운터 값(`CNT_VALUE`)은 매 클럭 하드웨어가 바꾸므로, `volatile` 없이 두 번 읽으면 컴파일러가 "값이 안 변했겠지" 하고 한 번만 읽고 재사용해버릴 수 있다. 하드웨어 레지스터 접근에는 거의 항상 `volatile`이 필요하다.
:::
...
`libregmap`은 한 파일이 아니라 **역할이 분리된 여러 파일**로 구성된다. 최상위 모듈 `lib_axil_regmap`(top)이 세 개의 구성요소 — 인터페이스(`axi4_lite_if`), 프로토콜 엔진(`lib_axil_protocol`), 배선(`lib_reg_glue`) — 를 안에 인스턴스화해 하나로 묶는다. 그리고 이 모든 모듈은 레지스터 레이아웃을 **패키지(`lib_axil_reg_pkg`) 한 곳에서만** 정의해 참조한다.

:::note[이 시스템은 AXI4-Lite 슬레이브이다]
`libregmap`(과 그 핵심인 `lib_axil_protocol`)은 AXI4-Lite **슬레이브(slave)** 로 동작한다. 즉 트랜잭션을 **시작하는 쪽이 아니라 받는 쪽**이다. 마스터(보통 CPU/PS)가 주소·데이터·요청을 보내면, 이 슬레이브는 그것을 받아 레지스터에 쓰거나 값을 돌려준다. 그래서 인터페이스 포트도 `axi4_lite_if.slave bus`처럼 **slave modport**로 받는다. 아래 타이밍 다이어그램에서 어느 신호를 마스터가 구동하고 어느 신호를 이 슬레이브가 구동하는지 색으로 구분해 두었다.
:::
...
| `lib_axil_reg_pkg.sv` | top 바깥 (참조) | 레지스터 **레이아웃의 단일 출처(SSOT, Single Source of Truth — 같은 정보를 여러 곳에 중복하지 않고 한 곳에서만 정의)**. 개수·오프셋·RO/RW 구분을 한곳에 정의. |

데이터 흐름을 한 문장으로 요약하면 이렇다. PS가 `axi4_lite_if`를 통해 `lib_axil_protocol`에 쓰기/읽기를 하면, 프로토콜 엔진은 쓴 값을 `reg_o`로 내보내고 읽을 값을 `reg_i`에서 가져온다. 그 사이를 `lib_reg_glue`가 이어주는데, 평범한 레지스터는 쓴 값을 그대로 돌려주고(echo), 읽기 전용 레지스터는 PL 하드웨어의 실제 상태값을 꽂아 넣는다. 그리고 이 모든 모듈이 어떤 주소가 몇 번 레지스터인지, 어느 것이 읽기 전용인지를 `lib_axil_reg_pkg` 하나만 보고 판단한다.

아래에서 프로토콜부터 시작해 네 파일을 차례로 뜯어본다.

## 1. AXI4-Lite 란 무엇인가

AXI4-Lite는 AMBA AXI4 규격에서 **버스트(burst)와 복잡한 기능을 걷어낸 경량 버전**이다. 여기서 버스트란 "주소 하나를 주면 거기서부터 여러 워드를 연속으로 쏟아붓는" 전송 방식으로, 대용량 메모리 전송에 쓰인다. AXI4-Lite는 이 버스트를 빼고 **한 번에 한 워드(여기서 워드는 데이터 폭 한 단위, 즉 32비트 또는 64비트)만** 주고받는다. 그래서 CPU가 주변장치의 제어 레지스터를 한 칸씩 읽고 쓰는 용도에 딱 맞고, "레지스터맵"을 만들 때 사실상 표준처럼 쓰인다.
...
AXI4-Lite는 신호들을 역할별로 5개 채널로 묶는다. 쓰기에 3개, 읽기에 2개이다.
...
:::note[`4'b0011` 같은 표기 읽는 법]
Verilog 상수 표기는 `폭'진법값` 꼴이다. `4'b0011`은 "4비트, 2진수(`b`), 값 `0011`"이라는 뜻이다. 마찬가지로 `2'b00`은 2비트 2진수 0, `4'hF`는 4비트 16진수(`h`) F(=`1111`), `8'h0C`는 8비트 16진수 0x0C이다. 이 글 곳곳에 이 표기가 나온다.
:::
...
구조는 크게 다섯 덩어리이다. 파라미터·포트 선언, 상수와 저장소, 헬퍼 함수 4개, **병렬 수신 쓰기 로직**, 읽기 로직. 쓰기와 읽기가 별도의 로직으로 분리돼 있다는 점이 핵심이다. AXI의 채널 독립성을 그대로 코드 구조로 옮긴 것이다.

:::note[8장부터는 한 단계 깊어진다]
5~7장(포트·상수·헬퍼 함수)까지는 어렵지 않다. 하지만 **8장 "쓰기 채널"부터는 두 채널을 동시에 받아 합류시키는 병렬 수신 구조**를 다루는데, 여기서 `always_comb`(조합 블록)와 `always_ff`(등록 블록)를 나눈 **2-process** 패턴이 등장한다. 간단히 말해, 조합 블록에서 "다음 클럭에 가질 값(next-state)"을 계산하고, 등록 블록에서 그 값을 클럭 엣지에 실제로 래치하는 2단 구조이다. SystemVerilog의 `always_comb`/`always_ff` 차이만 알고 있으면 충분히 따라올 수 있다.
:::
...
:::note[이전 버전과의 차이]
이전 버전은 쓰기가 직렬 FSM(상태 기계: `W_IDLE → W_DATA → W_RESP`로 한 단계씩 진행)이고 헬퍼가 3개였다. 이번 버전은 **병렬 수신 + 2-process 구조**로 바뀌고 정렬 검사(`addr_aligned`)가 추가되어 헬퍼가 4개가 됐다. 그 결과 더 빠르고(트랜잭션당 ~2클럭), 더 안전하다(비정렬 SLVERR). 또 회수·수신·커밋을 독립된 `if`로 분리하면서 back-to-back(쉬는 클럭 없이 트랜잭션을 연달아 보내는 것, 8장에서 정의) race(버그 #1)와 커밋 중복 latch(버그 #2)를 함께 잡았다. 자세한 건 8번에서 다룬다.
:::
...
:::note[`#( ... )`와 `parameter` 읽는 법]
모듈 이름 뒤의 `#( ... )` 안에 있는 `parameter`는 **인스턴스화할 때 바깥에서 바꿀 수 있는 설정값**이다. 예를 들어 데이터 폭을 64비트로 쓰고 싶으면 이 모듈을 가져다 쓰는 쪽에서 `DATA_WIDTH(64)`로 덮어쓰면 된다. 반면 모듈 안에서 `localparam`으로 선언한 값(6장에 나옴)은 **바깥에서 못 바꾸는 내부 상수**다. 즉 `parameter`는 "조절 손잡이", `localparam`은 "고정 나사"라고 보면 된다.
:::
...
- `IDX_WIDTH = $clog2(NUM_REGISTERS)`: 레지스터 인덱스를 표현하는 데 필요한 비트 수. `$clog2(N)`은 "N개를 구분하려면 몇 비트가 필요한가"를 주는 내장 함수다(올림 밑2 로그). 예: `$clog2(256)=8`. 즉 레지스터 256개의 번호 0~255를 8비트로 표현한다.
...
:::note[`genvar`/`generate`는 "하드웨어 복제기"다]
보통의 `for`가 "한 회로가 시간을 따라 반복 실행"하는 거라면, `generate`-`for`는 **합성 시점에 똑같은 회로를 N개 복붙**하는 것이다. 여기서는 `assign reg_o[gi] = regfile[gi];`를 레지스터 수(예: 256)만큼 펼쳐, 256개의 독립 연결선을 만든다. 반복 변수도 일반 `int`가 아니라 전용 `genvar`를 쓴다. `begin : g_expose`의 `g_expose`는 펼쳐진 블록에 붙이는 이름표(파형/계층에서 식별용)다.
:::

## 7. 헬퍼 함수 4개 — 줄 단위 설명

**AXI4-Lite의 R/W 주소의 유효성을 판정하고 변환**하는 함수들이다.

:::note[`function automatic ... ` 읽는 법]
SystemVerilog 함수는 `function 반환형 이름(입력들); ... return 값; endfunction` 꼴이다. `automatic`은 "호출할 때마다 지역 변수를 새로 잡는다"는 뜻으로, 같은 함수가 여러 곳에서 동시에 불려도 서로 값이 섞이지 않게 한다(하드웨어 함수에선 사실상 항상 붙인다). 반환형 `bit`는 1비트(참/거짓), `logic [IDX_WIDTH-1:0]`는 그 폭만큼의 비트 묶음이다. 이 함수들은 모두 클럭 없이 **입력만으로 즉시 값이 정해지는 조합 로직**이다.
:::
...
`wstrb`에 따라 바이트 단위로 골라 쓴다. 결과 `r`을 기존 값으로 시작해, `strb[i]`가 1인 바이트만 새 값으로 덮는다. 예를 들어 기존 `0xAABBCCDD`, 새 값 `0x11223344`, `strb=4'b0011`이면 하위 2바이트만 바뀌어 `0xAABB3344`가 된다.

## 8. 쓰기 채널 — 병렬 수신 구조

여기서부터가 이 설계의 가장 큰 특징이자, 글에서 가장 깊은 부분이다. 차근차근 보자.

### 왜 "병렬 수신"이 필요한가

2장에서 쓰기는 AW(주소) → W(데이터) → B(응답) 순서로 흐른다고 했다. 하지만 AXI4-Lite 규격은 **AW와 W의 도착 순서를 보장하지 않는다.** 마스터(CPU)는 셋 중 어떤 방식으로도 보낼 수 있다.

- AWVALID와 WVALID를 **같은 클럭**에 동시에 올리거나,
- AW를 **먼저** 보내고 W를 나중에 보내거나,
- W를 **먼저** 보내고 AW를 나중에 보내거나.

슬레이브는 이 세 경우를 **모두** 처리할 수 있어야 한다. 만약 "AW가 반드시 먼저 온다"고 가정하고 짜면, W가 먼저 오는 마스터를 만났을 때 동작이 깨진다.

그래서 이 설계는 **주소(AW)와 데이터(W)를 각각 독립적으로 받아두고, 둘이 다 모이면 그때 기록**하는 구조를 택했다. 이것이 "병렬 수신"이다. 먼저 온 채널을 일단 latch(붙잡아 저장)해두고, 나머지가 도착하면 합류시켜 한 번에 처리한다.

### 핵심 아이디어 — latch 후 합류

먼저, 먼저 온 채널을 기억해 둘 저장소가 필요하다.
...
`aw_taken`/`w_taken`은 "이 채널을 이미 받았다"는 플래그이고, `waddr_q`/`wdata_q`/`wstrb_q`는 받은 값을 보관하는 레지스터이다(`_q`는 클럭에 등록된 값이라는 관용 표기).

:::tip[`aw_taken`은 어떻게 1이 되고 0이 되나]
`aw_taken`은 **등록된 레지스터**라서, 매 클럭 조합 블록이 계산한 `n_aw_taken` 값을 `always_ff`가 래치해 결정한다. 즉 코드 한 곳에서 정해지는 게 아니라, 아래 2-process 로직의 세 군데가 `n_aw_taken`을 바꾼다. 미리 전체 그림을 잡고 가자(`w_taken`도 W 채널에 대해 똑같다).

- **set(0→1)**: `aw_hs`(AW 핸드셰이크 성립)가 일어난 클럭에 `n_aw_taken = 1`. "주소를 방금 붙잡았다"는 뜻. → 2-process의 (2) 수신
- **clear(1→0)**: 커밋이 끝났거나(주소·데이터를 다 써버림) 응답이 회수돼 트랜잭션이 끝난 클럭에 `n_aw_taken = 0`. → (1) 회수, (3) 커밋
- **유지**: 위 둘 다 아니면 현재 값 그대로(`n_aw_taken = aw_taken`). AW만 먼저 오고 W를 기다리는 동안 이 상태로 머문다.

아래 2-process 코드에서 `aw_taken`에 관여하는 줄만 추리면 이렇다(전체 코드는 곧 나온다).

```systemverilog
// [조합 always_comb] — n_aw_taken을 계산
n_aw_taken = aw_taken;        // (기본) 유지: 아무 일 없으면 현재 값 그대로
if (b_collect)    n_aw_taken = 1'b0;   // (1) 회수: 트랜잭션 끝 → clear
if (aw_hs)        n_aw_taken = 1'b1;   // (2) 수신: AW 붙잡음 → set
if (do_write_commit) n_aw_taken = 1'b0;// (3) 커밋: 주소 다 씀 → clear

// [등록 always_ff] — 계산된 값을 클럭에 새김
if (!rst_n) aw_taken <= 1'b0;          // 리셋: 0
else        aw_taken <= n_aw_taken;    // 매 클럭 n_aw_taken을 래치
```

세 `if`는 위에서 아래로 평가되므로, 같은 클럭에 수신과 커밋이 겹치면 마지막 줄((3) 커밋)이 이겨 `n_aw_taken=0`이 된다. "방금 받아서 바로 다 써버린" 경우라 0이 맞다. 리셋 시에는 `always_ff`에서 0으로 초기화된다. 그래서 `aw_taken=1`은 곧 "AW는 이미 받았고 W를 기다리는 중"을 의미한다.
:::

이어서, 매 클럭 상황을 판단하는 보조 신호들을 만든다.

:::note[이름에 붙은 `_hs`·`_q`와 `wire`/`logic` 구분]
- `_hs`는 handshake의 약자로, "이번 클럭에 그 채널의 valid·ready가 동시에 1인가"를 나타내는 1비트 신호다(`aw_hs`, `w_hs`, `ar_hs`).
- `_q`는 클럭에 등록된 레지스터(플립플롭) 값에 붙이는 관용 접미사다(`waddr_q` 등). "지난 클럭에 붙잡아 둔 값"이라는 뉘앙스.
- `wire`는 **조합 신호**(즉석 계산, 클럭 없음), `logic`으로 선언하고 `always_ff`에서 `<=`로 갱신하는 것은 **레지스터**(클럭마다 한 번씩 값이 바뀜)다. 이 둘을 구분하는 게 아래 코드를 읽는 핵심이다.
:::
...
:::note[이 네 줄은 모두 조합 `wire`다 — 레지스터가 아니다]
`have_aw`, `have_w`, `resp_slot_free`, `do_write_commit`은 전부 `wire`로, **그 자리에서 우변만 보면 값이 정해진다**(클럭과 무관, 매 순간 입력으로 즉시 계산). 뒤에 나오는 `aw_taken`·`bvalid` 같은 **레지스터**와 헷갈리지 말자. 레지스터는 "지난 클럭에 래치된 과거 값"이고, 이 `wire`들은 "그 과거 값 + 이번 클럭 입력"을 조합한 즉석 판단이다.
:::
...
여기서 `resp_slot_free`가 중요하다. 이 값도 위 코드 그대로 `(!bus.bvalid) || b_collect`로 정해진다. 단순히 `!bvalid`만 보지 않고, **이번 클럭에 응답이 회수되는 경우(`b_collect`)도 자리가 비는 것으로 본다.** 덕분에 응답을 돌려주는 바로 그 클럭에 곧장 다음 트랜잭션을 커밋할 수 있어 back-to-back이 막히지 않는다.

:::note[back-to-back이란]
back-to-back은 **트랜잭션을 쉬는 클럭 없이 등 맞대고(back to back) 연달아 보내는 것**을 말한다. 예를 들어 마스터가 첫 쓰기의 응답(B)을 받자마자 같은/다음 클럭에 두 번째 쓰기의 AW/W를 곧바로 올리는 경우다. 이때 슬레이브가 "이전 응답을 정리하느라" 한 클럭을 흘려보내면 그 사이 들어온 두 번째 요청을 놓칠 수 있다. 그래서 `resp_slot_free`가 "회수되는 그 클럭에 곧바로 다음 것을 받아들이도록" 설계된 것이고, 이게 8장 뒷부분 버그 #1과 직결된다. 이런 연속 전송을 멈춤 없이 처리하는 것이 슬레이브 성능과 정확성의 관건이다.
:::
...
이제 실제로 값을 받아 처리하는 로직이다. 이 쓰기 로직은 **하나의 `always_ff`에 모든 걸 욱여넣지 않고** 두 블록으로 나눴다.

- **조합 블록(`always_comb`)**: "다음 클럭에 각 레지스터가 가질 값"(next-state, 변수 앞에 `n_`를 붙임)을 계산만 한다. 실제 저장은 하지 않는다.
- **등록 블록(`always_ff`)**: 조합 블록이 계산한 `n_*` 값을 클럭 엣지에 그대로 래치한다. 실제 상태 변경은 여기서만 일어난다.

이렇게 나눈 이유가 핵심이다. 한 클럭에 **세 가지 사건이 동시에 일어날 수 있다.** ① 마스터가 이전 응답을 회수하고(`b_collect`), ② 새 AW/W가 도착하고(`aw_hs`/`w_hs`), ③ 주소·데이터가 다 모여 커밋이 일어나는 것. 이 셋을 **하나의 `if/else` 사슬에 넣으면 서로를 가로막는다.** 조합 블록에서 셋을 **독립된 `if` 세 개**로 풀어두면, 각 사건이 next-state에 따로따로 반영되어 충돌하지 않는다.
...
always_comb begin
    ...
    if (do_write_commit) begin
        // (주소 검사 후 regfile 기록 — 상세 코드는 바로 아래 '응답 코드' 절)
        n_bvalid   = 1'b1;                    // 응답 게시
        n_aw_taken = 1'b0;  n_w_taken = 1'b0; // 방금 쓴 주소·데이터 소비 → clear
    end
end
```

이 `(3)` 블록이 `aw_taken`/`w_taken`을 **0으로 clear하는 두 지점 중 하나**다(나머지 하나는 위 `(1)` 회수). 커밋이 일어나면 붙잡아 둔 주소·데이터를 다 써버린 것이므로 두 플래그를 비워, 다음 트랜잭션이 깨끗한 상태에서 시작하게 한다. 주소 검사·실제 기록의 상세는 바로 다음 절에서 본다.

맨 위에서 모든 `n_*`를 "현재 상태 유지"로 초기화하는 게 중요하다(`always_comb`에서 모든 경로의 값을 정해 latch가 생기지 않게 함). 그 뒤 세 부분이 차례로 필요한 것만 덮어쓴다. **(1) 응답 회수**: 마스터가 이전 응답을 받아가면(`b_collect`) bvalid를 내리고 두 채널을 다시 열어 다음 트랜잭션을 준비한다. **(2) 독립 수신**: AW나 W Handshake가 성립하면 각자 값을 latch하고, **그 채널만** ready를 내려 같은 값을 중복으로 받지 않게 한다. **(3) 커밋**: 주소·데이터가 모두 확보되면 레지스터에 쓴다.

핵심은 이 셋이 **상호배타 `if/else`가 아니라 독립된 세 개의 `if`** 라는 점이다. 그래서 회수가 ready를 막 열어준 **바로 그 클럭에 도착한 새 핸드셰이크도 (2)에서 정상적으로 latch된다.** 이것이 아래에서 설명하는 버그 #1 수정의 핵심이다.

:::note[버그 #1 — 응답 회수와 동시 수신의 race]
여기서 race(경쟁 상태)란 **두 사건이 같은 클럭에 겹쳐 일어날 때, 코드가 그중 하나만 처리하고 다른 하나를 놓쳐 결과가 틀어지는 상황**을 말한다. 이전 1-process 구현은 수신 latch가 `if (bvalid && bready) ... else ...`의 **else에 갇혀 있었다.** 그래서 응답을 회수하는 바로 그 클럭에는 수신 로직이 평가되지 않았다. 그 클럭에 마스터가 `ready=1`을 보고 다음 트랜잭션의 AW/W를 올리면 그 핸드셰이크가 통째로 버려져 데이터가 유실됐다(back-to-back에서 재현). 즉 "응답 회수"와 "새 수신"이 경쟁해 후자가 사라진 것이다. 2-process로 분리해 회수·수신·커밋을 독립된 `if`로 만들면서 이 race가 사라졌다.
:::
...
커밋 클럭에 `aw_taken`/`w_taken`을 `aw_hs`/`w_hs`로 보존하면, 동시 수신 후 즉시 커밋하는 경우 **'방금 커밋한 그 트랜잭션'의 핸드셰이크가 다시 latch**되어, 다음 클럭 `b_collect` 시 같은 데이터가 한 번 더 기록됐다(트랜잭션 1건당 `rf_we` 2회). 보통의 RW 레지스터라면 같은 값을 두 번 써도 결과가 같아 티가 안 나지만, **side-effect 레지스터**(쓰는 행위 자체가 동작을 일으키는 레지스터, 예: "쓰면 카운터 +1"이나 "쓰면 FIFO에서 한 칸 꺼냄" 같은 것)는 두 번 써지면 동작도 두 번 일어나 오작동한다. 커밋 클럭엔 ready=0이라 같은 클럭에 새 핸드셰이크가 성립할 수 없으므로, 커밋 시 두 채널을 모두 비우는 게 맞다.
:::
...
여기서 `aw_taken <= n_aw_taken;` 한 줄이 앞서 말한 "플래그의 일생"을 닫는다. 조합 블록의 세 `if`가 `n_aw_taken`을 set(수신)·clear(회수·커밋)·유지로 정하면, 이 줄이 그 값을 클럭마다 실제 `aw_taken`에 새겨 넣는다. 그래서 `aw_taken`이 "어디서 정해지나"의 답은 결국 **이 한 줄**이고, 그 입력값 `n_aw_taken`을 정하는 곳이 조합 블록 세 군데인 것이다.
...
:::note[이 DUT를 검증하려면]
DUT(Design Under Test)는 "검증 대상 설계", 즉 우리가 만든 이 레지스터맵을 가리킨다. 이런 레지스터맵은 보통 UVM 테스트벤치(검증 전용 환경)로 검증한다. 정상 쓰기/읽기, strobe 부분 쓰기, **비정렬 SLVERR**, 범위 밖 DECERR, AW/W 순서를 바꾼 경우(동시·AW우선·W우선), 백투백 트랜잭션 등을 시퀀스로 만들어 보낸 뒤, **스코어보드**(예상값과 실제 출력을 자동 대조하는 부품)로 비교하고, **커버리지**(테스트가 어떤 경우들을 실제로 밟았는지 집계하는 지표)로 빠진 케이스를 확인한다.
:::
...
5개 채널(AW/W/B/AR/R)의 신호가 전부 한 인터페이스 안에 선언돼 있다. 앞서 본 핸드셰이크 신호(`awvalid`/`awready` 등), 데이터(`wdata`, `rdata`), 응답(`bresp`, `rresp`)이 모두 여기 모여 있다. `awprot`/`arprot`는 그 접근이 특권/보안/명령 영역인지를 나타내는 3비트 보호 속성 신호로, 권한을 구분하는 시스템에서 쓰인다. 이 설계에서는 사용하지 않지만 AXI 규격상 포함된다.
...
`clocking block`은 **검증(테스트벤치) 전용** 기능이다. 신호를 언제 샘플링하고 언제 구동할지 타이밍을 명확히 정해, 테스트벤치와 DUT 사이의 경쟁 상태(race condition)를 막는다. `master_cb`는 테스트벤치가 마스터처럼 행동할 때, `monitor_cb`는 신호를 관찰만 할 때 쓴다. `default input #1step output #1ns`에서 `input #1step`은 "클럭 엣지 **직전**의 안정된 값을 읽는다"(샘플 시점), `output #1ns`는 "클럭 엣지 **1ns 뒤**에 구동한다"(드라이브 시점)는 뜻이다. 이렇게 읽기/쓰기 시점을 살짝 어긋나게 둬야 TB와 DUT가 같은 신호를 같은 순간에 건드려 값이 꼬이는 일을 피한다. 이 블록은 RTL 합성에는 영향을 주지 않고, 시뮬레이션에서만 동작한다.
...
256비트짜리 비트마스크로, RO 레지스터의 위치만 1로 세운다. `1 << IDX`는 "1을 IDX칸 왼쪽으로 민다", 즉 IDX번 비트만 1인 값을 만드는 관용 표현이다. 그것들을 `|`(OR)로 합치면 여러 RO 위치가 동시에 1인 마스크가 된다. 여기서는 `IDX_CNT_VALUE`(2번)와 `IDX_CNT_OVERFLOW`(3번) 비트가 켜진다. RO 레지스터를 추가할 때는 여기에 `| (1 << IDX_새것)` 한 항만 추가하면 된다. 이 마스크를 보고 glue가 어느 레지스터에 하드웨어 값을 꽂을지 결정한다.

## 13. lib_reg_glue — reg_i 배선 전담

`lib_axil_protocol`은 AXI 프로토콜만 처리하고, "읽을 값을 어디서 가져올지"는 신경 쓰지 않는다. 그 결정을 이 `lib_reg_glue`가 전담한다. 앞서 regmap이 읽기 때 `reg_i`에서 값을 가져온다고 했는데, **그 `reg_i`를 채우는 게 바로 이 모듈**이다.
...
이 블록이 glue의 핵심이다. 동작은 두 단계이다. 먼저 **모든 레지스터를 echo로 연결**한다. `reg_i[k] = reg_o[k]`를 전체에 적용해, 기본적으로 "쓴 값이 그대로 읽힌다"가 된다(RW 동작). 그다음 **RO 레지스터만 덮어쓴다**. `reg_i[IDX_CNT_VALUE]`에는 하드웨어의 실제 카운터 값을, `IDX_CNT_OVERFLOW`에는 오버플로 횟수를 꽂는다. 이러면 그 두 주소는 CPU가 뭘 쓰든 무시되고 하드웨어 값이 읽힌다.

여기서 순서가 중요하다. `always_comb` 안에서 같은 `reg_i[2]`에 두 번 대입하면 **나중 대입이 이긴다(last-write-wins).** for 루프가 먼저 `reg_i[2] = reg_o[2]`로 깔아두지만, 바로 아래 RO 주입이 그 위를 덮으므로 최종값은 하드웨어 값이 된다. 그래서 "전체 echo 먼저, RO 덮어쓰기 나중"이라는 이 순서 자체가 동작의 핵심이다.

:::tip[이 구조의 장점]
RW 레지스터가 몇 개든 `for` 한 줄로 처리되고, RO는 추가한 개수만큼만 코드가 늘어난다. 레지스터를 추가할 때 패키지에서 오프셋과 RO 마스크만 정의하고, RO면 glue에 주입 한 줄만 더하면 된다. 배선 로직이 레지스터 수에 휘둘리지 않는다.
:::
...
"UVM" 검증환경에서는 설계한 AXI4-Lite를 어떻게 UVM 관점에서 검증할 수 있는지 다룰 예정이다.[UVM으로 AXI4-LITE 검증하기](/rtl/uvm-verification/)
