...
## 1. 이 회로가 뭘 하는가 — 회로도 한 장으로

비트 이야기를 하기 전에 **무엇을 만드는지**부터 못 박자. 1편에서 유도한 수식은 이것이었다.

$$y = ((c_3\mu + c_2)\mu + c_1)\mu + c_0$$

새 샘플이 올 때마다 최근 샘플 4개로 계수 $c_0 \sim c_3$를 만들고, "샘플 사이 어디를 볼 것인가"를 나타내는 μ(0–1)를 세 번 곱해 보간값 $y$를 얻는다. 이 수식을 **RTL의 `always_ff` 문장 하나도 빠짐없이** 회로로 옮긴 것이 아래 그림이다.

**이 글 전체가 이 그림 한 장을 읽는 과정이다.** 1장에서는 그림의 **모양**(무엇이 어떻게 연결됐나)을 읽고, 2장에서는 선 옆에 회색으로 적힌 **숫자**(몇 비트인가)를 읽는다. 3–5장은 그 숫자들이 어디서 나왔는지의 유도이고, 6–7장은 이 그림대로 만든 회로가 정말 맞는지의 확인이다.

![datapath 전체 회로도 — RTL 신호 이름과 비트폭](../../../../../assets/catmull-rom/catmull_rom_farrow_schematic.svg)

:::note[기호 읽는 법 — 이것만 알면 된다]
| 기호 | 뜻 |
|---|---|
| 네모 상자 | **레지스터**(플립플롭). 클럭마다 값을 한 번 붙잡아 두는 1클럭짜리 기억 소자 — 상자를 하나 지날 때마다 결과가 **1클럭 늦어진다** |
| `Tₛ` 상자 | shift register의 한 칸. 샘플 **1개** 지연 |
| ⊗ | 곱하기 |
| ⊕ | 더하기 |
| 상자 옆 `c0_s2` 같은 글자 | **RTL 신호 이름 그대로**. 뒤에 붙은 `_sN`은 "파이프라인 **N번째 단**의 레지스터"라는 뜻 |
| 선 옆 회색 `CW` 같은 글자 | 그 선을 흐르는 숫자의 **비트폭**. 2장의 주제 — 지금은 무시하고 모양만 보자 |
:::

처음 보면 복잡해 보이지만 **크게 세 덩어리**다. 왼쪽 세로줄 → 가운데 가지들 → 아래 가로줄, 이 순서로 읽으면 된다.

### ① 왼쪽 세로줄 — 최근 샘플 4개를 들고 있는다 (shift register, s1)

입력 `x[n]`이 `Tₛ` 상자를 하나씩 통과하며 아래로 밀린다. 그래서 위에서부터 $x_2, x_1, x_0, x_{-1}$ — 최신 샘플부터 가장 오래된 샘플까지 **네 개의 탭(tap)이 동시에 존재**한다. 새 샘플이 들어오면 전부 한 칸씩 밀리고 가장 오래된 것이 빠진다.

보간이란 "점 4개를 보고 그 사이를 추측하는 일"이므로(1편), 항상 최근 4개가 동시에 필요하다. **이 4개가 보간에 쓰는 창(window)** 이다. 각 탭에서 오른쪽으로 뻗은 가로선(탭 버스)이 아래 branch들을 먹인다 — shift register **하나**를 4개 branch가 공유한다.

### ② 가운데 세로 가지 4개 — 계수를 만든다 (branch filter, s2)

가운데에 세로 가지들이 서 있다. 여기서 계수 $c_3, c_2, c_1$을 만들고, 맨 오른쪽 세로줄이 $c_0$($=x_0$, 계산 없이 복사)다. 각 가지는 위의 탭 버스에서 필요한 샘플을 받아 ⊗(상수 곱)와 ⊕(덧셈)로 조합한다. 예를 들어 $c_2 = x_{-1} - 2.5x_0 + 2x_1 - 0.5x_2$.

⊗ 옆에 `0.5`, `−1.5`, `2`, `−2.5` 같은 상수가 붙어 있는데 — **여기엔 곱셈기가 하나도 없다.**

:::tip[⊗로 그렸지만 곱셈기가 아니다 — 이 그림에서 가장 중요한 지점]
계수가 전부 **1/2의 배수**라서 그렇다. 하드웨어에서 2의 거듭제곱을 곱하는 것은 **배선을 비스듬히 연결하는 것**일 뿐이다.
...
곱셈 기호로 그린 것은 **수식에서 무슨 값을 곱하는지 보여주기 위해서**일 뿐, 합성하면 배선과 덧셈기만 남는다. 그래서 이 영역 전체가 **곱셈기 0개**다.

만약 계수가 0.7, 1.3 같은 임의의 실수였다면 이 자리에만 곱셈기가 12개 필요했을 것이다. **Catmull-Rom을 고른 하드웨어적 이유가 이것**이다.
:::

### ③ 아래 가로줄 — μ를 세 번 곱한다 (Horner 사슬, s3–s8)

맨 아래 가로줄에서 ⊗ 와 ⊕ 가 번갈아 나오며 $y = ((c_3\mu + c_2)\mu + c_1)\mu + c_0$ 를 안쪽 괄호부터 계산한다.

:::tip[여기 ⊗ 3개만이 진짜 곱셈기다]
가로줄의 ⊗ 세 개는 **μ를 곱한다.** μ는 매 출력마다 달라지는 **변수**라 상수 shift로 대체할 수 없다 — 진짜 곱셈기가 필요하다.

$$y = ((\underbrace{c_3 \times \mu}_{\text{s3}} + c_2)\underbrace{\times \mu}_{\text{s5}} + c_1)\underbrace{\times \mu}_{\text{s7}} + c_0$$

**곱셈기 3개.** 합성하면 DSP48E2(FPGA에 박혀 있는 전용 곱셈 하드웨어) 3개로 매핑된다(7장 실측). 위쪽 branch의 ⊗ 들과 생김새는 같지만 **성격이 완전히 다르다**:

| | 위쪽 branch의 ⊗ | 아래 Horner의 ⊗ |
|---|---|---|
| 곱하는 값 | **상수** (0.5, 2, 2.5…) | **변수** μ |
| 실제 하드웨어 | shift + add | **진짜 곱셈기 (DSP)** |
| 개수 | 0개 | **3개** |
:::

### 그림의 나머지 두 요소 — μ 레일과 "그냥 기다리는" 레지스터들

여기서 잠깐, 이 회로는 전체 계산을 8단계(s1–s8)로 쪼개 **컨베이어 벨트처럼 흘려보내는 파이프라인**이다(왜 쪼개는지는 5-3에서). 단계를 쪼갠 대가로, **나중에 쓰일 재료는 자기 차례까지 실어 날라야 한다.** 그림의 나머지 요소들이 전부 그 "운반"이다.

**아래 μ 레일.** 가장 아래 가로선이 μ이고, 세 곱셈기로 각각 화살표가 올라간다. 중간중간 레지스터(`mu_s1`~`mu_s6`)가 박혀 있는데, **곱셈이 s3·s5·s7 세 시점에 일어나므로 같은 세트의 μ를 그 시점까지 들고 가야** 하기 때문이다.

**오른쪽에 세로로 쌓인 상자들.** `c0_s2 → c0_s3 → … → c0_s7` 이 여섯 개나 쌓여 있다. 첫 상자(`c0_s2`)는 계수를 만드는(정확히는 $c_0 = x_0$이니 복사해 두는) 레지스터고, 나머지 다섯 개는 아무 계산도 하지 않는다 — $c_0$ 는 Horner의 마지막 덧셈(s8)에서야 쓰이므로 자기 차례까지 **그냥 기다린다.** 마찬가지로 $c_1$ 은 s6까지, $c_2$ 는 s4까지 기다린다.

:::note[계수가 합류하는 순서 = Horner 괄호 순서]
$$y = ((c_3\mu + \underbrace{c_2}_{\text{s4}})\mu + \underbrace{c_1}_{\text{s6}})\mu + \underbrace{c_0}_{\text{s8}}$$

안쪽 괄호부터 계산하니 $c_3 → c_2 → c_1 → c_0$ 순으로 합류한다. 그림에서 **계수가 실려가는 레지스터 개수**가 그대로 이 순서다.

| 계수 | 소비되는 단 | 실려가는 레지스터 |
|---|---|---|
| $c_3$ | s3 (첫 곱셈) | 0개 — 만들자마자 사용 |
| $c_2$ | s4 | 1개 |
| $c_1$ | s6 | 3개 |
| $c_0$ | s8 (마지막) | **5개** |

계수를 실어나르지 않으면 Horner 순서가 깨져 계산이 틀린다. **파이프라인의 대가**가 이 레지스터들이다.
:::

### 왜 이 구조인가 — 계수에 μ가 없다 (Farrow)

이 그림이 통째로 1편 3장에서 유도한 **Farrow 구조**다. 핵심은 **계수를 만드는 위쪽(①②)에 μ가 등장하지 않는다**는 것. 그래서 계수는 새 샘플이 올 때만 계산하면 되고, μ가 아무리 자주 바뀌어도 다시 하는 건 Horner 3번뿐이다. 지금까지를 요약하면:

| 덩어리 | 그림 위치 | 하는 일 | 단 |
|---|---|---|---|
| shift register | 왼쪽 세로줄 | 최근 샘플 4개($x_{-1} \sim x_2$) 보관 | s1 |
| branch filter | 가운데 가지들 | 계수 4개 생성 — **곱셈기 0개** | s2 |
| Horner | 아래 가로줄 | μ를 3번 곱해 $y$ 계산 — **곱셈기 3개** | s3–s8 |
| μ 레일 | 맨 아래 | 보간 위치(0–1)를 각 곱셈 시점까지 운반 | s1–s6 |

그림과 코드는 1:1이다. 예를 들어 $c_2$ 가지는
...
이고, 그림에서 $x_{-1}$·$x_0$·$x_1$·$x_2$ 네 탭이 상수 ⊗ 를 거쳐 ⊕ 로 모이는 모양이 이 한 줄이다. `2.5x_0` 는 `(x0<<<1) + (x0>>>1)` 로 분해되어 있다.

모양은 다 읽었다. 이제 질문은 하나로 좁혀진다. **이 선들 위를 흐르는 숫자를 몇 비트로 나를 것인가?**

## 2. Q 포맷 — 그림의 회색 숫자를 읽는 법
...
### 같은 그림을 다시 — 이번엔 선의 "굵기"를 본다

1장 그림을 다시 보자. 이번에는 모양이 아니라 **선 옆의 회색 라벨**을 읽는다.

![datapath 전체 회로도 — 회색 라벨이 각 선의 비트폭이다](../../../../../assets/catmull-rom/catmull_rom_farrow_schematic.svg)

선에 붙은 사선 틱(`/`)과 라벨이 "이 선은 몇 비트짜리 버스인가"를 말한다. 라벨은 전부 RTL의 파라미터 이름이고, FPGA 기본 구성에서의 값은 그림 맨 아래 범례와 같다.

| 라벨 | FPGA 값 | Q 포맷 | 그림의 어느 선 |
|---|---|---|---|
| `IN_W` | **14** | Q1.13 | 입력 `x[n]` — ADC 샘플 |
| `XW` | 15 | Q1.14 | 탭 4개의 가로 버스 |
| `CW` | **18** | Q4.14 | 계수, 그리고 모든 ⊕ 의 출력 |
| `MU_W` | **18** | Q0.18 | μ 레일 |
| `PW` | 37 | Q5.32 | ⊗(μ 곱셈) 직후 |
| `OUT_W` | **16** | Q2.14 | 출력 `y[n]` — 포화 후 |

그림을 따라가다 보면 질문이 두 개 떠오를 것이다. **왜 입력은 14비트인데 첫 탭부터 15비트인가?** 그리고 **왜 곱셈 뒤엔 37비트로 부풀었다가 매번 18비트로 돌아오는가?** 이 두 답이 2장의 나머지 전부다.

### 질문 1 — 14가 15로 늘어나는 곳: `align_frac()`

그림에서 입력 선(`IN_W`)이 첫 `Tₛ` 상자에 들어가기 직전, 폭이 `XW`로 1비트 늘어난다. 이 1비트를 붙이는 것이 **`align_frac()`** 함수다 — 입력 Q1.13을 내부 Q1.14로 바꾼다. 그림에 상자로 그리지 않은 이유는, FPGA 구성에서는 `<<1` 한 줄이라 **배선일 뿐 게이트도 안 쓰기** 때문이다.

```systemverilog
function automatic logic signed [XW-1:0] align_frac (input logic signed [IN_W-1:0] v);
    if (SH >= 0) align_frac = XW'(v) <<< SH;              // 늘리거나 그대로 (무손실)
    else begin
...
**내부 포맷 Q1.14는 원인이 아니라 결과다.** "`>>1`을 정확히 하겠다"는 결정이 먼저고, `SH=+1`도 `align_frac()`의 `<<1`도 전부 거기서 따라 나왔다.
...
### 질문 2 — 18이 37로 부풀었다 돌아오는 곳: Horner의 폭 맥박

이제 그림의 아래 가로줄을 폭만 보며 왼쪽에서 오른쪽으로 따라가 보자. 세부를 걷어내면 이렇다.

```
입력      탭      계수     ⊗       ⊕      ⊗       ⊕      ⊗       ⊕      출력
 14  →   15  →   18  →  [37] →   18  →  [37] →   18  →  [37] →   18  →   16
                         곱셈      rnd     곱셈     rnd     곱셈      rnd    포화
```

**패턴이 하나뿐이다.** μ(18비트)를 곱할 때마다 18b가 37b(`PW`)로 부풀고, `round_half_up()`(반올림)가 다시 18b(`CW`)로 되돌린다. 그게 3번 반복된다 — 그림에서 ⊗ 뒤가 `PW`, ⊕ 뒤가 `CW`인 것이 바로 이 맥박이다.
...
| `align_frac(din)` | s1 조합 | 15b | Q1.14 | 소수부 1비트 확보 (`SH=+1`, `<<1`) |
...
| `v1_s4` | s4 레지스터 | 18b | Q4.14 | `round_half_up(p3) + c2` |
| `p2_s5` | s5 레지스터 | 37b | Q5.32 | `v1 × mu` |
| `v2_s6` | s6 레지스터 | 18b | Q4.14 | `round_half_up(p2) + c1` |
| `p1_s7` | s7 레지스터 | 37b | Q5.32 | `v2 × mu` |
| `y_full` | s8 조합 | 18b | Q4.14 | `round_half_up(p1) + c0` |
...
**입력 폭과 내부 정밀도는 별개다.** 14비트 ADC를 받으면서 내부를 9비트로 계산할 수 있고(ASIC 구성), 그 변환을 `align_frac()`이 한다.
:::
...
그림 두 번째 줄의 **X = x≪1** 은 2장에서 본 `align_frac()`이다. 덕분에 branch 4개의 계산에는 **반올림 오차가 전혀 없고**, 오차가 생기는 곳은 Horner의 곱셈 3곳뿐이다.
...
- `CW'( )`는 폭 맞춤 캐스팅이다. CW는 L1 분석이 강제한 폭(정수부 4 + 소수부 14 = 18비트)으로, 이 폭 안에서는 위 연산이 절대 오버플로우하지 않음이 **3장에서 수학적으로 보장**되어 있다.
...
```systemverilog
// round_half_up(p, SHIFT): p / 2^SHIFT 를 round-half-up 으로.
// 자를 비트 수(SHIFT)를 인자로 명시해 호출부에서 의도가 드러나게 했다.
function automatic logic signed [PW-1:0] round_half_up
    (input logic signed [PW-1:0] p, input int SHIFT);
    logic signed [PW-1:0] t;
    if (SHIFT <= 0) begin
        round_half_up = p;                      // 자를 소수부가 없음
    end else begin
        t = p + (PW'(1) <<< (SHIFT-1));         // 0.5 LSB 를 더하고
        round_half_up = t >>> SHIFT;            // 산술 shift = 나누기
    end
endfunction

// 호출부: 곱셈으로 늘어난 소수부(MU_W)만큼 되돌리고 CW 로 캐스팅
...
그리고 실전에서 만난 버그 하나. 내부 정밀도를 입력보다 낮게 잡는 구성(ASIC의 `DP_FRAC=9`)에서는 `align_frac()`이 입력을 **반올림으로 줄인다**(`SH=−4`). 그런데 +최대값 근처 입력이 반올림되며 **정확히 +1.0**이 되는 경우가 있었다.
...
| | 어디 | 언제 | 왜 |
|---|---|---|---|
| `align_frac()`의 반올림 | 입력단 | `SH<0` (ASIC만) | 입력을 내부 정밀도로 **줄일 때** |
| `round_half_up()`의 반올림 | Horner 각 단 | **항상** | 곱셈으로 부푼 폭(37b)을 되돌릴 때 |
...
rnd = @(p) floor((p + 2^(P.MU_W-1)) ./ 2^P.MU_W);   % RTL의 round_half_up() 과 동일
c0 = X0;  c1 = shr(X1-Xm1);
...
`floor(v/2)`가 RTL의 `>>> 1`과, `floor((p+2^17)/2^18)`이 RTL의 `round_half_up()`와 정확히 같은 정수 연산임에 주목하자. 이 함수가 벡터 파일(입력, μ, 기대 출력)을 만들고, SystemVerilog testbench가 같은 입력을 RTL에 넣어 출력을 한 줄씩 비교한다.
...
:::note[표의 용어]
**Fmax** = 이 회로가 버티는 최대 클럭 주파수. **LUT** = FPGA의 기본 로직 조각(작을수록 좋다), **FF** = 플립플롭(레지스터), **CARRY8** = 덧셈용 캐리 체인 블록, **DSP** = 전용 곱셈 하드웨어(DSP48E2). "곱셈기 3개" 주장이 맞다면 DSP 열이 3이어야 한다.
:::
...
| 비트폭 | FPGA / ASIC 최적점 | Q4.14 / Q4.9 (면적 43% 절감, 7장 실측) |
...
5. **비용 곡선이 최적점을 정한다** — 같은 성능 분석에서 FPGA(계단 비용)와 ASIC(선형 비용)의 답이 달라진다. 최적화는 목적함수와 제약을 명시해야 성립한다.
6. **bit-accurate 모델과 코너 벡터** — "많이 돌렸다"보다 "어떤 경로를 찔렀다"가 검증이다. guard bit 버그는 랜덤 2만 개가 아니라 코너 8개가 잡았다.
...
wire w_in_hs = w_both & w_adv;              // 그때만 파이프 진입
```

**④ 파라미터가 범용화됐다.** 본문 코어는 입력을 Q1.13으로 고정 가정했지만, 라이브러리는 `IN_INT`(정수부 비트)를 파라미터로 받아 **Q(n).m 어떤 포맷이든** 처리한다. 기본값 `IN_INT=1`이 본문의 −1–+1 정규화 입력이다. `DP_FRAC` 위에는 **대역폭 B별 권장값 표**를 주석으로 달았다 — 4장 knee 분석의 결론을 코드 옆에 붙여, 쓰는 사람이 knee를 다시 유도하지 않아도 값을 고를 수 있게 했다.

:::note[knee 계산을 RTL에 넣지 않은 이유]
"B를 파라미터로 받아 내부에서 `DP_FRAC`을 자동 계산하면 되지 않나?" 실제로 그렇게도 만들어봤다. 하지만 knee 공식은 `log10`을 포함한 **실수 연산**이라, `real` 파라미터를 쓰면 합성 툴 호환성이 갈린다(오픈소스 툴에서 override가 막히는 것을 확인했다).

더 근본적으로는 **역할이 다르다.** knee 공식은 "이 신호에 몇 비트가 필요한가"라는 **설계 결정**이고, RTL은 "주어진 비트폭으로 회로를 만드는" 일이다. 이 둘을 섞으면 RTL이 특정 신호 가정(평탄 스펙트럼)에 종속된다. 나사는 규격을 받고, 어떤 나사를 쓸지는 설명서가 안내한다 — 그래서 **표는 주석으로, 값은 파라미터로** 두었다.
:::
...
//
// DP_FRAC: internal datapath fraction bits. Choose it from the platform and
// the input signal's normalized bandwidth B (cycles/sample, 0..0.5):
//
//   FPGA : DP_FRAC = 14 regardless of B -- the DSP48 fraction is fixed-size,
//          so the extra precision is free; use it.
//   ASIC : DP_FRAC = knee(B) + 1, where knee is where SQNR stops limiting the
//          total SNR. The multiplier is gates, so pick the smallest that keeps
//          the SNR floor. Reference table (flat-spectrum signal):
//
//            bandwidth B          floor SNR   knee   DP_FRAC (=knee+1)
//            0.030 < B <= 0.038    ~40 dB      11        12   (ultra narrow)
//            0.039 < B <= 0.075    ~35 dB      10        11   (very narrow)
//            0.076 < B <= 0.151    ~30 dB       9        10
//            0.152 < B <= 0.301    ~26 dB       8         9
//            0.302 < B <= 0.500    ~23 dB       7         8   (widest)
//
// Input format: signed Q(IN_INT).(IN_W-IN_INT). IN_INT is the integer bits
// including sign; the default IN_INT=1 is the usual normalized -1..+1 sample.
//=============================================================================
`default_nettype none

module lib_axis_interp_catmull #(
    parameter int IN_W      = 14, // input sample total width
    parameter int IN_INT    = 1,  // input integer bits incl. sign; Q(IN_INT).(IN_W-IN_INT)
    ...
    // ------------------------------------------------------------------ params
    localparam int IN_FRAC = IN_W - IN_INT;                 // input fractional bits
    localparam int SH  = DP_FRAC - IN_FRAC;                 // frac shift (int part irrelevant)
    localparam int XW  = DP_FRAC + IN_INT + ((SH < 0) ? 1 : 0); // int part carried + guard
    localparam int CW  = XW + 3;       // C0~C3 BW: +3 from L1-norm |c2|max=6 (log2(6)<3)
    ...
    // align_frac: re-align input Q(IN_INT).(IN_FRAC) to internal Q(IN_INT).DP_FRAC.
    // SH>=0 widens (lossless <<); SH<0 narrows with round-half-up (1 guard bit).
    function automatic logic signed [XW-1:0] align_frac (input logic signed [IN_W-1:0] v);
    ...
    // round_half_up(p, SHIFT): return round(p / 2^SHIFT) with round-half-up.
    // SHIFT is the number of low (fractional) bits to drop, made explicit at
    // each call site instead of being implied. SHIFT<=0 is a no-op passthrough.
    // Self-contained (depends only on p, SHIFT, PW) so it lifts into a shared
    // package unchanged. Callers pass MU_W: each Horner product grew the
    // fraction by exactly MU_W bits, so dropping MU_W restores Q4.DP_FRAC.
    function automatic logic signed [PW-1:0] round_half_up
        (input logic signed [PW-1:0] p, input int SHIFT);
        logic signed [PW-1:0] t;
        if (SHIFT <= 0) begin
            round_half_up = p;                      // nothing to drop
        end else begin
    ...
    wire  signed [CW-1:0]    w_y_full;
    assign w_y_full = CW'(round_half_up(r_p1_s7, MU_W)) + r_c0_s7;
