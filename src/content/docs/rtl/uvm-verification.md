...
- **내려가는 흐름(자극)**: `axil_test` → `axil_sequences`(시퀀스) → `axil_sequencer` → `axil_driver` → DUT. "이런 신호를 보내라"가 위에서 아래로. (`axil_sequences`는 "무엇을 보낼지" 정하는 시나리오로, 10장에서 자세히 다룬다.)
...
for (int off = 1; off <= 3; off++) begin
    bit [31:0] a = base*4 + off;     // 0x05, 0x06, 0x07 ... (어긋남)
...
`axil_test`는 조직의 **최상위**다. 두 가지 일만 한다 — 조직(env)을 세우고, 시퀀스(지시서)를 내려보낸다.
...
```systemverilog
// 📄 tb_lib_axil_regmap.sv — 검증환경과 회로를 잇는 최상단 (핵심만)
...
1. `axil_test`가 `axil_smoke_seq`를 실행 → 시퀀스가 `write_reg(0x04, 0x1234)` 전표를 발급
...
추가로 Vivado를 활용해 UVM 환경을 설정하고 시뮬레이션을 통해 Coverage 결과까지 뽑는 방법을 다음 링크에서 다룬다.
...
복사해서 바로 쓸 수 있도록 검증환경 전체 코드를 싣는다. 컴파일은 패키지가 모든 클래스를 `include`하므로, 시뮬레이터에 인터페이스(`clk_rst_if.sv`, `axi4_lite_if.sv`) → `axil_pkg.sv` → `tb_lib_axil_regmap.sv` 순서로(또는 도구 자동 정렬) 넘기면 된다. DUT 측 코드(`lib_axil_protocol.sv` 등)는 앞 글 [AXI4-Lite 슬레이브 레지스터맵](/rtl-library/axi4lite-regmap/)의 부록에 있다.
...
//  레지스터 파일은 '범위 안=OKAY, 범위 밖=DECERR'만 존재(RO/SLVERR 없음).
//----------------------------------------------------------------------
...
class axil_coverage extends uvm_subscriber #(axil_seq_item);    // uvm_subscriber 상속 = "전표를 구독(수신)하는 집계자"
    ...
    // DUT와 일치 (256 레지스터, 영역 = 256*4 = 1024바이트, 0x000~0x3FF)
    localparam int NUM_REGISTERS = 256;                       // 레지스터 개수(집계 기준, DUT와 일치해야 함)
    localparam int REGION_BYTES  = NUM_REGISTERS * 4;         // 유효 주소 범위(바이트)
    ...
    covergroup cg;                                            //
        ...
        // 주소 영역: 범위 안(여러 구간) / 범위 밖
        //   범위 안을 첫/중간/끝 구간으로 쪼개, 레지스터 파일 전체를 건드렸는지 확인
...
//  주소 정책: 범위 안 = OKAY, 범위 밖 = DECERR (RO/SLVERR 없음).
//----------------------------------------------------------------------
