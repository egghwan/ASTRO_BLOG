...
앞뒤 4개 샘플(`x[n-1]`~`x[n+2]`)로 **3차 곡선의 계수 c0~c3를 만든다.** 이 계수들은 Catmull-Rom이라는 표준 보간 공식에서 나온 것으로, c1은 기울기, c2는 곡률처럼 곡선의 모양을 점점 정교하게 잡는다. 마지막 줄이 **Farrow의 핵심**: 계수에 μ를 차례로 곱해 넣는 다항식이다. `((c3*mu+c2)*mu+c1)*mu+c0` 형태(호너 방식)로 써서 곱셈 횟수를 줄였다.
...
6개 샘플(`pts`가 그 상대 위치 -2~+3)을 쓴다. 이중 루프는 **라그랑주 보간의 표준 공식**이다. 각 샘플 i마다 "그 샘플만 1이고 나머지 위치에선 0이 되는" 기저 다항식 L을 만들어(안쪽 j 루프), 샘플값에 곱해 모두 더한다. 점을 더 많이 보니 곡선을 더 정확히 따라가지만, 그만큼 곱셈과 지연이 늘어난다.
...
정할 비트는 셋. **신호 비트 Bx**, **μ 해상도 Bmu**, **누적기 폭 Bacc**.
...
function w = worst_fixed(interp, x, Bx, Bmu, q)
    g=64; w=inf;
    for mu = 0.1:0.1:0.9
        muq = q(mu, Bmu);                   % μ를 Bmu 비트로
        xq  = q(x, Bx);                     % 입력을 Bx 비트로
        yi  = ideal_at(x, mu);              % 정답은 float 기준
        yf  = q(interp(xq, muq), Bx);       % 보간 후 출력도 Bx로
...
fprintf('--- Bx 민감도 (Bmu=7) ---\n');
for Bx = 5:10
    fprintf('Bx=%2d : %.1f dB\n', Bx, worst_fixed(@farrow_6tap, x35, Bx, 7, q));
end
fprintf('--- Bmu 민감도 (Bx=8) ---\n');
for Bmu = 4:8
    fprintf('Bmu=%2d : %.1f dB\n', Bmu, worst_fixed(@farrow_6tap, x35, 8, Bmu, q));
end
```

```
--- Bx 민감도 (Bmu=7) ---       --- Bmu 민감도 (Bx=8) ---
Bx= 5 : 23.2 dB                  Bmu= 4 : 28.9 dB
Bx= 6 : 29.0 dB                  Bmu= 5 : 34.0 dB
Bx= 7 : 34.6 dB                  Bmu= 6 : 37.9 dB
Bx= 8 : 39.9 dB                  Bmu= 7 : 39.9 dB
Bx= 9 : 43.9 dB                  Bmu= 8 : 40.9 dB
Bx=10 : 45.4 dB
```

![비트 폭 민감도 분석](../../../assets/interp-bitwidth.png)

**Bx는 1비트당 약 5~6dB씩** 또박또박 오른다(양자화 기본 법칙). 8bit에서 합격선(37.9dB)을 넘는다. **Bmu도 비슷한 비중**이라 6bit에서 합격선에 닿는다.

:::tip[두 잡음원의 균형이 효율이다]
Bx만 잔뜩 늘리고 Bmu를 작게 두면, 이번엔 Bmu가 천장이 돼 점수가 안 오른다(Bmu=4면 28.9dB에서 막힘). 반대도 마찬가지. **두 잡음원이 비슷한 크기일 때** 자원을 가장 알뜰하게 쓴다. 한쪽만 키우는 건 한쪽 바퀴만 큰 자전거를 만드는 격이다.
:::
...
for Bx = 5:10
    for Bmu = 4:8
        s = worst_fixed(@farrow_6tap, x35, Bx, Bmu, q);
        if s >= PASS
            cost = Bx*6 + Bmu;        % 6-tap → ×6 (곱셈기 크기 비례)
            if cost < minCost, minCost=cost; best=[Bx Bmu s]; end
        end
    end
end
Bacc = best(1) + best(1) + ceil(log2(6)) + 2;   % 누적기 (공식)
fprintf('최적: Bx=%d, Bmu=%d, SQNR=%.1f dB, 누적기=%d bit\n', ...
        best(1), best(2), best(3), Bacc);
```

```
최적: Bx=8, Bmu=6, SQNR=37.9 dB, 누적기=21 bit
```

:::note[누적기는 탐색하지 않고 공식으로]
Bx, Bmu는 실험으로 찾지만, 누적기는 **넘치지만 않으면 되므로 공식으로** 정한다. 8비트끼리 곱하면 16비트(Bx+Bx), 6개를 더하면 약 3비트(log₂6) 더 불어나고, 안전 여유 2비트를 더해 **21비트**. 탐색이 아니라 계산 대상이다.
:::
...
fprintf('-- Bx 민감도 (Bmu=7) --\n');
for Bx = 5:10
    fprintf('  Bx=%2d : %.1f dB\n', Bx, worst_fixed(@farrow_6tap, x35, Bx, 7));
end
fprintf('-- Bmu 민감도 (Bx=8) --\n');
for Bmu = 4:8
    fprintf('  Bmu=%2d : %.1f dB\n', Bmu, worst_fixed(@farrow_6tap, x35, 8, Bmu));
end
...
for Bx = 5:10
    for Bmu = 4:8
        s = worst_fixed(@farrow_6tap, x35, Bx, Bmu);
        if s >= PASS
            cost = Bx*6 + Bmu;
            if cost < minCost, minCost = cost; best = [Bx Bmu s]; end
        end
...
Bacc = best(1)*2 + ceil(log2(6)) + 2;
fprintf('\n>> 최적: Bx=%d, Bmu=%d, SQNR=%.1f dB, 누적기=%d bit\n', ...
        best(1), best(2), best(3), Bacc);
...
function y = farrow_6tap(x, mu)                                    % Lagrange 6-tap
    pts=[-2 -1 0 1 2 3]; sh=[2 1 0 -1 -2 -3]; N=numel(x); y=zeros(1,N);
    for i=1:6
        L=ones(1,N);
        for j=1:6
...
% --- 고정소수점 최악 SQNR ---
function w = worst_fixed(interp, x, Bx, Bmu)
    q = @(v,B) round(v*2^(B-1))/2^(B-1);
    g=64; w=inf;
    for mu = 0.1:0.1:0.9
        muq=q(mu,Bmu); xq=q(x,Bx);
        yi=ideal_at(x,mu); yf=q(interp(xq,muq), Bx);
        e=yf(g:end-g)-yi(g:end-g); sg=yi(g:end-g);
        w=min(w, 10*log10(sum(sg.^2)/sum(e.^2)));
    end
end
